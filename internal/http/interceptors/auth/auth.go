...
func New(m map[string]interface{}, unprotected []string) (global.Middleware, error) {
	...
	// set defaults
	if len(conf.TokenChain) == 0 {
		conf.TokenChain = []string{"header"}
	}
	...
	tokenChain := make([]auth.TokenStrategy, 0, len(conf.TokenChain))
	for _, strategy := range conf.TokenChain {
		f, ok := tokenregistry.NewTokenFuncs[strategy]
		if !ok {
			return nil, fmt.Errorf("token strategy not found: %s", strategy)
		}

		tokenStrategy, err := f(conf.TokenStrategies[strategy])
		if err != nil {
			return nil, err
		}
		tokenChain = append(tokenChain, tokenStrategy)
	}
	...
	chain := func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			ctx, err := authenticateUser(w, r, conf, tokenChain, tokenManager, tokenWriter, credChain, isUnprotectedEndpoint)
			if err != nil {
...
func authenticateUser(w http.ResponseWriter, r *http.Request, conf *config, tokenChain []auth.TokenStrategy, tokenManager token.Manager, tokenWriter auth.TokenWriter, credChain map[string]auth.CredentialStrategy, isUnprotectedEndpoint bool) (context.Context, error) {
	ctx := r.Context()
	...
	for _, tokenStrategy := range tokenChain {
